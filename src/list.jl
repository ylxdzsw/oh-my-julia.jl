...
Δ(x::Vector{T}) where T = Tuple{T,T}[(x[i+1],x[i]) for i in 1:length(x)-1]
Δ(f::Function, x::Vector) = [f(x[i+1],x[i]) for i in 1:length(x)-1]
Δ(f::Function, ::Type{T}, x::Vector) where T = T[f(x[i+1],x[i]) for i in 1:length(x)-1]
...
(>>)(a, b::AbstractVector) = pushfirst!(b, a)
